...
- **⚠️ CI deploy is gated on IAM:** the CI service account (in `FIREBASE_SERVICE_ACCOUNT_TTUBOC_ATTENDANCE`) lacks deploy permission (`firebaserules :test` → 403). CI **builds green**; the deploy step is `continue-on-error` and prints the exact roles to grant. **Until granted, deploy via local `firebase deploy` (the proven path).** Roles needed: firebaserules.admin, datastore.indexAdmin, cloudfunctions.admin, run.admin, iam.serviceAccountUser, firebasehosting.admin, artifactregistry.admin, cloudbuild.builds.editor.
- **Prod Anthropic key:** the local deploy bundles `.env.local` into the SSR fn (Next loads it at runtime) → parse works. CI provisions it the same way once IAM is granted.
