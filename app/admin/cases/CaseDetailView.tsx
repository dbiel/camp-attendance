...
import { AddTimelineNote } from './[id]/AddTimelineNote';
...
/**
 * Renders a single report's detail. Two presentations:
 *   - `variant="page"` (default): standalone route at /admin/cases/[id] — back
 *     link, resolving navigates to the board. Used on mobile + deep links.
 *   - `variant="panel"`: the desktop split view's right column — a close button
 *     instead of a back link, resolving calls `onResolved` (clear + refresh the
 *     left list) rather than navigating, and prior-report taps stay in the panel
 *     via `onSelectCase`.
 */
export function CaseDetailView({
  ...
  async function resolve() {
    ...
    try {
      ...
      if (res.ok) {
        // Panel: hand control back to the board (clear selection + refresh the
        // left list). Page: navigate back to the board as before.
        if (variant === 'panel' && onResolved) onResolved();
  ...
  const Root = isPanel ? 'div' : 'main';
  const rootClass = isPanel ? '' : 'mx-auto max-w-2xl p-4';

  if (loadError && !detail) {
    return (
      <Root className={isPanel ? 'p-1 text-sm text-[var(--text-3)]' : 'p-4 text-sm text-[var(--text-3)]'}>
        {isPanel && <PanelClose onClose={onClose} />}
        <p>{loadError}</p>
  ...
  /** Prior-report tap: stay in the panel on desktop, else navigate. */
  function openPrior(id: string, e: React.MouseEvent) {
    if (isPanel && onSelectCase) {
      e.preventDefault();
      onSelectCase(id);
    }
  }
    ...
    <Root className={rootClass}>
      {isPanel ? (
        <div className="mb-2 flex items-center justify-between gap-2">
          <span className="flex min-w-0 items-center gap-1.5 text-sm font-medium text-red-700">
          ...
          <button
            ...
            className="shrink-0 rounded border border-[var(--glass-border)] px-2 py-1 text-sm text-[var(--text-2)] hover:bg-black/5"
          >
            ✕ Close
          </button>
        </div>
      ...
      <h1 className={isPanel ? 'text-lg font-bold' : 'mt-2 text-xl font-bold'}>{c.student_name}</h1>
          ...
          <div className="mt-2 flex flex-col gap-3">
            {prior_cases.map((p) => (
              <div key={p.id} className="glass-card p-3 text-sm">
                <div className="flex items-baseline justify-between">
                  <Link
                    href={`/admin/cases/${p.id}`}
                    onClick={(e) => openPrior(p.id, e)}
...
/** Close button shown alone in the panel's loading/error states. */
  ...
  return (
    <div className="mb-2 flex justify-end">
      <button
        ...
        onClick={onClose}
        aria-label="Close report"
        className="rounded border border-[var(--glass-border)] px-2 py-1 text-sm text-[var(--text-2)] hover:bg-black/5"
      >
        ✕ Close
      </button>
    </div>
